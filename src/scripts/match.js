...
async function initCardMatchingGame() {
    ...
    const suits = ["♠️", "♥️", "♦️", "♣️"];
    const deck = [...suits, ...suits].sort(() => Math.random() - 0.5);
    const cards = [];
    let flippedCards = [];
    let matchedPairs = 0;
    let lives = 3;
    ...
    const startX =
        (app.screen.width - (4 * (cardWidth + cardSpacingX) - cardSpacingX)) /
        2;
    const startY =
        (app.screen.height - (2 * (cardHeight + cardSpacingY) - cardSpacingY)) /
        2;
    ...
    function createCard(index, suit) {
        ...
        card.y = startY + Math.floor(index / 4) * (cardHeight + cardSpacingY);
